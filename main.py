...
def notify_admin(user_id, username, full_name, message_text):
    ...
    reply_markup = {
        "inline_keyboard": [
            [{"text": "✍️ پاسخ دادن", "callback_data": f"reply:{user_id}"}],
            [{"text": "🚫 بلاک", "callback_data": f"block:{user_id}"}]
        ]
    }

    tg_send_message(ADMIN_ID, text, reply_markup=reply_markup, parse_mode="Markdown")
...
def handle_admin_message(message):
    ...
    if text == "/users":
        ...
        tg_send_message(admin_id, response, parse_mode="Markdown")
        return
    ...
    if text.startswith("/unblock"):
        try:
            ...
            tg_send_message(admin_id, "استفاده: `/unblock <user_id>`", parse_mode="Markdown")
        return
