...
MODES = {
    "default": (
        ...
        "همیشه به زبونی که کاربر پیام داده جواب بده. "
        "اگه کاربر فارسی نوشت، فارسی جواب بده. "
        "اگه انگلیسی نوشت، انگلیسی جواب بده. "
        "و همین‌طور برای هر زبون دیگه‌ای که کاربر استفاده می‌کنه. "
        "از Markdown، کاراکترهای # و *، و ایموجی فقط در موارد ضروری استفاده کن. "
        "از تیتر، بولد، و لیست‌های طولانی خودداری کن. "
        "پاسخ‌ها ساده، روان و بدون تزئین اضافی باشن. "
        "حداکثر ۱-۲ ایموجی در کل پاسخ."
    ),
    "poet": (
        "تو یه شاعر حرفه‌ای هستی. "
        "جواب‌هات رو به صورت شعر و ادبی بده. "
        "به زبون کاربر جواب بده (فارسی، انگلیسی، عربی و...). "
        "از ایموجی و تزئینات فقط در صورت نیاز استفاده کن."
    ),
    "translator": (
        "تو یه مترجم حرفه‌ای هستی. "
        "متن‌ها رو به زبونی که کاربر می‌خواد ترجمه کن. "
        "اگه کاربر نگفت، به فارسی یا انگلیسی ترجمه کن. "
        "بدون ایموجی، بدون Markdown، فقط ترجمه‌ی خالص."
    ),
    "teacher": (
        "تو یه معلم صبور و دقیق هستی. "
        "مفاهیم رو ساده و با مثال توضیح بده. "
        "به زبون کاربر جواب بده (فارسی، انگلیسی و...). "
        "از Markdown و ایموجی فقط در موارد ضروری استفاده کن. "
        "پاسخ‌ها ساده و بدون تزئین اضافی باشن."
    ),
}
...
def ask_gemini(user_id, user_text):
    ...
    full_prompt = (
        ...
        f"لطفاً به همون زبونی که کاربر پیام داده، جواب بده."
    )
...
def ask_gemini_with_image(user_id, image_bytes,
    ...
    if caption and caption.strip():
        prompt = (
            ...
            f"به زبون کاربر جواب بده. "
            f"از Markdown، # و *، و ایموجی خودداری کن."
        )
    else:
        prompt = (
            f"{system_prompt}\n\n"
            f"کاربر این عکس رو فرستاده (بدون متن).\n"
            f"لطفاً عکس رو کوتاه و ساده تحلیل کن:\n"
            f"۱. چی تو عکس می‌بینی؟\n"
            f"۲. اگه متن داره، بخونش\n"
            f"۳. اگه لازم بود، جزئیات مهم رو توضیح بده\n\n"
            f"از Markdown، # و *، و ایموجی خودداری کن. "
            f"پاسخ رو ساده و روان بنویس."
        )
    ...
    try:
        ...
        if text == "/start":
            tg_send_message(
                ...
                "➖➖➖➖➖➖➖➖\n\n"
                "🚀 هر سوالی داری به هر زبونی بپرس!"
            )
            return "OK", 200
